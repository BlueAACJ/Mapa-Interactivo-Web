...
def test():
    ...
    puntuacion = datos['value']
    usuario = session['username']

    # Traemos el id del usuario de la base de datos
    db.execute('SELECT id FROM Puntuacion WHERE usuario = %s', (usuario,))
    id_usuario = db.fetchone()

    if id_usuario:
        db.execute("UPDATE puntuacion SET puntuacion = %s WHERE usuario = %s and id = %s", (puntuacion, usuario,id_usuario,))
        mysql.connection.commit()
